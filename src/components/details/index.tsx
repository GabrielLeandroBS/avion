			...
			<div className='c-details__container'>
				<figure className='c-details__figure'>
					{loading ? (
						...
						<LazyLoadImage alt='product image' src={`${details.image.data[0].attributes.url}`} effect='blur' />
					)}
				</figure>

				<section className='c-details__content'>
					<h1 className='c-details__title'>{loading ? <Skeleton height={80} /> : details.title}</h1>
					<p className='c-details__price'>
						{loading ? (
							<Skeleton height={40} width={100} />
						) : (
							<>
								<label hidden htmlFor='quantity'>
									Preço:
								</label>
								<NumericFormat
									id='quantity'
					...
					<div>
						<p className='c-details__description'>{loading ? <Skeleton height={80} /> : details.description}</p>
					</div>
					...
					<div className='c-details__buttons'>
						...
						<ButtonLink title={'Go to Cart'} href={'/baskets'} color={'white'} isLoading={loading} />
					</div>
				</section>
			</div>
		</section>
