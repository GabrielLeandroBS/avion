...
const ProductsHighlighted: React.FC = () => {
	...
	const [highlightedProduct, setHighlightedProduct] = useState<highlightedProps>({
		title: '',
		products: {
			data: [
				{
	...
	return (
		<section className='c-products'>
			<h2 className='c-products__title'>{loading ? <Skeleton height={50} /> : title}</h2>
			<div className='c-products__wrapper'>
				{loading
					? Array.from({ length: 4 }, (item, index) => <CardProduct key={`highlighted-${index}`} isLoading={true} />)
					: ''}
				{!loading &&
