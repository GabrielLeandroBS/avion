		...
		(set) => ({
			...
			addItemsIntoCart: (params: AddToCartInitialProps) => {
				set((state: InitialProps) => ({
					...
					totalOfItemsInCart: state.totalOfItemsInCart + parseFloat(params.price),
					itemsAllocatedInsideCart: [...state.itemsAllocatedInsideCart, params],
				}))
			},

			UpdatingItemInsideTheCart: (params: UpdateInitialCartProps, getCart: CartContentProps) => {
				set((state: InitialProps) => ({
					quantityOfItemsInCart: state.quantityOfItemsInCart + 1,
					totalOfItemsInCart: state.totalOfItemsInCart + parseFloat(params.price),
			...
			removeCompleteItemFromCart: (params: RemoveCartInitialProps) =>
				set((state: InitialProps) => ({
					totalOfItemsInCart: state.totalOfItemsInCart - params.price * params.quantityOfItemsInCart,
					quantityOfItemsInCart: state.quantityOfItemsInCart - params.quantityOfItemsInCart,
